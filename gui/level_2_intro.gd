extends Control
## Standalone pre-level screen for **Level 2** (mirrors **`level_1_intro`**; art from **`level 2/intro/`**).

const _NEXT_SCENE := "res://game_level_2.tscn"
const _CONTENT_BOTTOM_PAD_PX := 36.0
const _BTN_GAP_PX := 10.0
const _CONTINUE_BTN_W := 150.0
const _BTN_H := 44.0
const _DIALOGUE1: Texture2D = preload("res://level 2/intro/dialogue1.png")
const _DIALOGUE2: Texture2D = preload("res://level 2/intro/dialogue2.png")
...
func _go_next() -> void:
	...
	if err != OK:
		push_error("level_2_intro: failed to load %s (error %d)" % [_NEXT_SCENE, err])
