extends Node2D
## Single sparrow: flies into view, lands on `Sparrow_fly4`, then loops **2× full idle** → **1× full hop** → repeat.

const _FLY_FRAMES: Array[Texture2D] = [
	preload("res://level/props/birds/Sparrow/fly/Sparrow_fly1.png"),
	preload("res://level/props/birds/Sparrow/fly/Sparrow_fly2.png"),
	preload("res://level/props/birds/Sparrow/fly/Sparrow_fly3.png"),
	preload("res://level/props/birds/Sparrow/fly/Sparrow_fly3_1.png"),
	preload("res://level/props/birds/Sparrow/fly/Sparrow_fly4.png"),
]

const _LAND_TEX: Texture2D = preload("res://level/props/birds/Sparrow/fly/Sparrow_fly4.png")

const _IDLE_FRAMES: Array[Texture2D] = [
	preload("res://level/props/birds/Sparrow/idle/Sparrow_idle1.png"),
	preload("res://level/props/birds/Sparrow/idle/Sparrow_idle2.png"),
	preload("res://level/props/birds/Sparrow/idle/Sparrow_idle3.png"),
]

const _HOP_FRAMES: Array[Texture2D] = [
	preload("res://level/props/birds/Sparrow/hop/Sparrow_hop1.png"),
	preload("res://level/props/birds/Sparrow/hop/Sparrow_hop2.png"),
]
...
const _HOP_FRAME_SEC := 0.32
const _IDLE_ROUNDS_BEFORE_HOP := 2
## Uniform scale for all sparrow art on screen (1/3 of native texture; double prior 1/6).
const _SPARROW_TEX_SCALE := 1.0 / 3.0
...
func _ready() -> void:
	...
	_sprite.scale = Vector2(_SPARROW_TEX_SCALE, _SPARROW_TEX_SCALE)
	add_child(_sprite)
...
func begin_flight(delay_sec: float) -> void:
	...
	var land_x := randf_range(rect.position.x + rect.size.x * 0.2, rect.position.x + rect.size.x * 0.8)
	var land_y := _query_ground_y_at_x(land_x, rect.position.y)
...
func _process_ground(delta: float) -> void:
	...
	while true:
		var frame_sec := _HOP_FRAME_SEC if _ground_is_hop else _IDLE_FRAME_SEC
		var frames: Array[Texture2D] = _HOP_FRAMES if _ground_is_hop else _IDLE_FRAMES
		if frames.is_empty():
			return
...
func _on_ground_cycle_wrapped() -> void:
	if _ground_is_hop:
		...
		_idle_rounds_done = 0
		_ground_frame_acc = 0.0
	elif _idle_rounds_done < _IDLE_ROUNDS_BEFORE_HOP:
		_idle_rounds_done += 1
		if _idle_rounds_done >= _IDLE_ROUNDS_BEFORE_HOP:
			_ground_is_hop = true
			_ground_frame_i = 0
			_ground_frame_acc = 0.0
