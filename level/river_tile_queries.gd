...
static func player_started_river_plummet(tm: TileMap, player: CharacterBody2D) -> bool:
	if not player.has_method(&"can_trigger_river_submersion"):
		return false
	if not bool((player as Object).call(&"can_trigger_river_submersion")):
		return false
...
static func _feet_bottom_center_cell_is_river(tm: TileMap, player: CharacterBody2D) -> bool:
...
static func _feet_on_cypress_river_tile_floor(tm: TileMap, player: CharacterBody2D) -> bool:
...
static func player_feet_below_viewport(player: CharacterBody2D, margin_px: float = 80.0) -> bool:
	var cam := (player as Node).get_node_or_null(^"Camera") as Camera2D
...
static func feet_world_y(player: CharacterBody2D) -> float:
	var cs := player.get_node_or_null(^"CollisionShape2D") as CollisionShape2D
