...
## Level 2 Beale: Paulo finish, AC façade upgrade, foreground decor, bag prop (2026-05-11)

**Paulo (Level 2 exit):** **`level 2/level_2.tscn`** adds **`FinishLine`** (**`Node2D`**) with **`level 2/paulo_goal.gd`** and child **`Square`** (**`Sprite2D`**, `centered = false`). Same interaction contract as Feena: proximity + **`drop_seed` + `action_suffix`**, skip while **`Player.is_holding_trash()`**, calls **`game_controller.present_level_complete()`**. Hint **“Talk to Paulo”** uses **`top_level`** and fixed theme font overrides so it does not scale with Paulo. Sprite height targets **Lawrence’s on-screen `Sprite2D` AABB height** (so **`game_level_2.tscn`** Player root scale is included), then **`_PAULO_HEIGHT_VS_LAWRENCE`** (default **1.25**) so Paulo reads taller; **`call_deferred`** reapplies after the player exists. Animation: **wipe** frames **`wipe1`–`wipe6`** loop **4×**, then **idle** **`idle1`–`idle4`** once, repeat; timings multiplied by **`_SPRITE_TRANSITION_SLOWDOWN`** (**1.6** = **60% slower** transitions). **`_apply_paulo_sprite_visual()`** runs before the editor early-return so Paulo is visible in the **2D editor**.
...
| Path | Role |
|------|------|
| **`level 2/paulo_goal.gd`**, **`level 2/paulo_goal.gd.uid`** | Paulo goal: hint, interact, wipe/idle animation, Lawrence-relative height, editor sprite setup. |
...
| **`level 2/level_2.tscn`** | **`FinishLine` / Paulo**; **`BagProp`**; **AC** instances (**`ExtResource` `ac_old_unit.tscn`**); **T8/T9/B17/B31/B32** **`z_index`**; **`T9`** script ref; removed inlined **Old AC** textures / subresource in favor of the AC scene. |
