...
| **`gui/score_hud.gd`** | Mission header `Button` updated to non-white hot/idle text; glow removed; focus/hover refresh logic retained. (The **`mission_hud_panel`** node group for **`SoundToggle`** layout is documented under [Pause menu refresh…](#pause-menu-refresh-global-sound-toggle-and-map-hub-copy-2026-05-13).) |
| **`gui/pause_menu.tscn`** | Same-day follow-up: see [Pause menu refresh…](#pause-menu-refresh-global-sound-toggle-and-map-hub-copy-2026-05-13) (this table originally noted **`VBoxContainer.separation = 8`** only). |
...
| **`gui/pause_menu.tscn`** | See [Pause menu refresh…](#pause-menu-refresh-global-sound-toggle-and-map-hub-copy-2026-05-13) (spacing **8** was the first note in this batch; the pause pass grew afterward). |
...
## Theme hub-aligned buttons, Level 3 aquifer placeholder, and Level 2 completion captions (2026-05-12)

### Global **`gui/theme.tres`** — **`Button`** matches **`map/map.tscn`** hub cards

- **`StyleBoxFlat`** fill (**light blue**), **2 px** gold border, **8 px** corner radius, **12 px** content margins, shadow — same values as **`StyleBoxFlat_level_button`** on the map hub.
- **Font:** black **`font_color`**, **`font_outline_color`** **`(0.38, 0.54, 0.73)`**, **`outline_size` `1`**; normal / hover / pressed / disabled / focus all use that stylebox.
- **Consumers:** any root using **`theme.tres`** inherits this (**`gui/pause_menu.tscn`**, **`gui/level_complete_screen.tscn`**, **`gui/river_splash_menu.tscn`**, **`gui/world_map.tscn`**, **`map/memphis_aquifer_placeholder.tscn`**). **`map/map.tscn`** hub buttons keep their **scene-local** overrides and still match visually.

### **`gui/level_complete_screen.tscn`**

- Removed **`theme_override_colors/font_outline_color`** and **`theme_override_constants/outline_size`** from **Retry** / **Go to Map** / **Next Level** so those buttons use the shared **theme** (map-style fill and outline).

### Level 3 — **Memphis Aquifer** hub placeholder

- **`level 3/Lswim.png`** — full-screen background art for the aquifer teaser.
- **`map/memphis_aquifer_placeholder.tscn`** + **`map/memphis_aquifer_placeholder.gd`**: **`TextureRect`** **`KEEP_ASPECT_COVERED`**; overlay **title** (**Level 3: Memphis Aquifer**) and **body** copy; **`CenterTexts`** occupies the **bottom quarter** of the viewport (**`anchor_top = 0.75`**), **`mouse_filter` ignore** so it does not block input; **`VBox`** (**`separation = 10`**) with **Back to map** below the copy (**`size_flags_horizontal = SIZE_SHRINK_CENTER`**), **`SpacerBeforeBack`** for extra space above the button; small **`offset_bottom`** inset from the screen edge.
- **`map/map.gd`**: **`MEMPHIS_AQUIFER_PLACEHOLDER_SCENE`**, **`_on_memphis_aquifer_pressed`**, **`_ready`** wiring if not already connected.
- **`map/map.tscn`**: **`MemphisAquiferButton`** **`pressed`** → **`_on_memphis_aquifer_pressed`**.

### Level 2 — level-complete **stars** and **captions**

- **`gui/level2_mission_goals.gd`**: **`level2_completion_stars_and_message(tree, gl)`** — maps **roof / AC / butterfly** mission flags to **0–3** stars and one of the authored **Beale** strings (plus fallbacks for other partial combinations); constants **`_L2_COMPLETE_MSG_*`**.
- **`level 2/level.gd`**: **`get_completion_stars_and_message(tree)`** delegates to that helper when **`use_memphis_mission_hud`** is **`true`**; otherwise **`{ stars: 0, message: "" }`**. **`game.gd`** **`present_level_complete`** already calls **`get_completion_stars_and_message`** for non-Memphis-L1 levels.

### Files touched

| Path | Change |
|------|--------|
| **`gui/theme.tres`** | Map-style default **`Button`** appearance. |
| **`gui/level_complete_screen.tscn`** | Strip button overrides so theme applies. |
| **`map/memphis_aquifer_placeholder.tscn`** | New scene: **`Lswim`** background + overlay UI + **Back**. |
| **`map/memphis_aquifer_placeholder.gd`** | **Back** → **`map/map.tscn`**. |
| **`map/map.gd`**, **`map/map.tscn`** | Aquifer hub navigation + signal. |
| **`level 3/Lswim.png`** (+ **`.import`**) | Background texture asset. |
| **`gui/level2_mission_goals.gd`** | **`level2_completion_stars_and_message`**. |
| **`level 2/level.gd`** | **`get_completion_stars_and_message`** implementation. |
...
- **2026-05-13:** keyboard help under the pause buttons, dimmer alpha **0.7**, **splash** **Resume** / singleplayer **Restart**, global **Sound OFF/ON** ( **`SoundToggle`** ), and related files — see [Pause menu refresh, global sound toggle, and map hub copy (2026-05-13)](#pause-menu-refresh-global-sound-toggle-and-map-hub-copy-2026-05-13).
